            ...
            return this.each(function() {
                ...
                function updateCode() {
                    ...
                    timer = setTimeout(function(){
                        ...
                        preloader.fadeIn(100, async function(){
                            ...
                            try {
                                ...
                                // -------------------------------
                                //  🔥 CONVERSIONE FORM DATA → JSON
                                // -------------------------------
                                let jsonData = {};
                                sendata.split("&").forEach(pair => {
                                    const [k, v] = pair.split("=");
                                    if (k) jsonData[k] = decodeURIComponent(v || "");
                                });

                                // -------------------------------
                                //  🔥 INVIO CHUNK AL NUOVO ENDPOINT
                                // -------------------------------
                                let msg = await sendChunks(relative + "ajax/chunk-process.php", jsonData);
                                ...
                                alert_placeholder.html("Errore durante invio chunk: " + error.message);
                                $('.toast').toast('show');
                                preloader.fadeOut('slow');
                            }
                ...
                function saveCode() {
                    ...
                    preloader.fadeIn(100, async function(){
                        ...
                        try {
                            ...
                            // Convertiamo il payload nel formato richiesto da chunk-create.php
                            let payload = {
                                create: dataObj
                            };

                            // ✨ Invio CHUNK → chunk-create.php → ricostruzione → create.php
                            let msg = await sendChunks(relative + "ajax/chunk-create.php", payload);

                            if (!msg) {
                                alert_placeholder.html("Errore: risposta vuota da chunk-create");
                                $('.toast').toast('show');
                                preloader.fadeOut('slow');
                                return;
                            }
                            ...
                            if (getdata.error) {
                                ...
                                var downloadlinks =
                                    '<button class="btn btn-default svgtopng" data-path="' + filepath + '">' + pngIcon + '</button>' +
                                    '<a href="#" class="btn btn-default d-none preload-png">' + loadIcon + '</a>' +
                                    '<a class="serve-png d-none" href="' + filepath + '.png" download="' + getdata.basename + '.png">PNG</a>';

                                downloadlinks +=
                                    '<a class="btn btn-default serve-svg" href="' + filepath + '.svg" download="' + getdata.basename + '.svg">' + svgIcon + '</a>';

                                if (!dataObj.nopdf) {
                                    downloadlinks +=
                                        '<a target="_blank" class="btn btn-default" href="' + relative + 'pdf/?f=' + getdata.basename + usefonts + '">' + pdfIcon + '</a>';
                                }

                                downloadlinks += '<button class="btn btn-default print">' + printIcon + '</button>';
                            ...
                            alert_placeholder.html("Errore durante invio chunk: " + err.message);
                            $('.toast').toast('show');
                            preloader.fadeOut('slow');
                        }
                    });
                ...
                // Spezza una stringa in chunk (default 120 KB)
                function splitIntoChunks(str, size = 120 * 1024) {
                    let chunks = [];
                ...
                // Invia i chunk a un endpoint PHP
                // async function sendChunks(url, jsonObj) {
                //     const jsonStr = JSON.stringify(jsonObj);
                //     const chunks = splitIntoChunks(jsonStr);
                ...
                //         if (!response.ok) {
                //             throw new Error("Errore chunk " + i);
                //         }
                //     }

                //     // Ultima risposta sarà quella completa di process.php/create.php
                ...
                async function sendChunks(url, jsonObj) {
                    const jsonStr = JSON.stringify(jsonObj);
                    const chunks = splitIntoChunks(jsonStr);
                    let lastResponse = null; // Variabile per memorizzare l'ultima risposta
                    ...
                    for (let i = 0; i < chunks.length; i++) {
                        ...
                        // Se questo è l'ultimo chunk, la sua risposta è quella che ci interessa
                        if (isLastChunk) {
                            lastResponse = await response.text();
                        } else {
                            // Per i chunk intermedi, puoi comunque leggere la risposta per assicurarti che sia OK
                            await response.json(); 
                        }
                    }

                    // Rimuovi completamente la seconda chiamata fetch ridondante

                    // Restituisci la risposta dell'ultimo chunk
                    return lastResponse;
                }
